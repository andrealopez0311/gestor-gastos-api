...
def crear_gasto_comun(data: GastoComunRequest, user_id: int = Depends(get_user)):
    ...
    cur.close()
    conn.close()
    return {"mensaje": "Gasto fijo añadido", "id": gasto_id}
...
def eliminar_gasto_comun(gasto_id: int, user_id: int = Depends(get_user)):
    ...
    hogar_id = get_hogar_id_opcional(cur, user_id)
    if hogar_id:
        cur.execute("DELETE FROM gastos_comunes WHERE id = %s AND hogar_id = %s", (gasto_id, hogar_id))
    else:
        cur.execute("DELETE FROM gastos_comunes WHERE id = %s AND usuario_id = %s AND hogar_id IS NULL", (gasto_id, user_id))
    conn.commit()
    cur.close()
    conn.close()
...
def editar_gasto_comun(gasto_id: int, data: EditarGastoComunRequest, user_id: int = Depends(get_user)):
    ...
    if hogar_id:
        ...
        cur.execute("""
            SELECT descripcion, importe, fecha, categoria_id
            FROM gastos_comunes WHERE id = %s AND usuario_id = %s AND hogar_id IS NULL
        """, (gasto_id, user_id))
    ...
    if hogar_id:
        ...
        """, (descripcion, importe, fecha, categoria_id, gasto_id, user_id))

    conn.commit()
    cur.close()
    conn.close()
    return {"mensaje": "Gasto fijo actualizado"}